    ...
    for i_step in 1:sim.nsteps
        ...
        if (i_step - 1) % sim.write_every == 0
            if i_step == 1
                ...
                write_xyz(sim.ofile, [string(length(sim.masses), "\nV = ", energy, " E_kin = ", get_kinetic_energy(sim.velocities, sim.masses), " T = ", get_temperature(sim.velocities, sim.masses) * conversion(:au_temperature, :kelvin))], [sim.labels], [sim.coords], append=true)
            end
        end
    end
end
...
struct AndersenThermostat <: AbstractIntegrator
    dt::Float64
    T::Float64
    AndersenThermostat(dt::Float64, T::Float64) = new(dt, T * conversion(:kelvin, :au_temperature))
end
...
LangevinThermostat(dt::Float64, T::Float64, α::Float64, masses::Vector{Float64}) = LangevinThermostat(dt * conversion(:fs, :au_time), T * conversion(:kelvin, :au_temperature), α, (1.0 .- 0.5 * α * dt * conversion(:fs, :au_time) ./ masses) ./ (1.0 .+ 0.5 * α * dt * conversion(:fs, :au_time) ./ masses), sqrt.(1.0 ./ (1.0 .+ 0.5 * α * dt * conversion(:fs, :au_time) ./ masses)), sqrt(2.0*α*T*dt * conversion(:fs, :au_time)), Normal(0.0, sqrt(2.0*α*T*dt * conversion(:fs, :au_time))), rand(Normal(0.0, sqrt(2.0*α*T*dt * conversion(:fs, :au_time))), (3, length(masses))))
...
function randomly_place_atoms_on_3d_grid(side_length::Union{Float64, Int}, num_atoms::Int, spacing::Float64, min_dist::Float64)
    coords = zeros(3, num_atoms)
    possible_coords = LinRange((-0.5 * side_length):spacing:(0.5 * side_length))
    for i in 1:num_atoms
        vec = possible_coords[rand(1:length(possible_coords), 3)]
        while all(==(1), norm.(eachcol(vec .- coords)) .> min_dist)
            vec = possible_coords[rand(1:length(possible_coords), 3)]
        end
        coords[:, i] = vec
    end
