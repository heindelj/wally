...
function write_reaxff_cgem_input(ofile::String, coords::AbstractMatrix, labels::AbstractVector{String}, index_and_charge::Union{Dict{Int, Int}, Nothing}=nothing)
	...
	open(ofile, "w") do io
		...
		write(io, string(length(labels), " atoms\n"))
		write(io, string(length(labels_unique) + 1, " atom types\n")) #+1 for the electron shell
		write(io, "\n-100 100 xlo xhi\n")
		write(io, "-100 100 ylo yhi\n")
		write(io, "-100 100 zlo zhi\n")
		write(io, "\nMasses\n")
		...
		for (i, vec) in enumerate(eachcol(coords))
			...
			count += 1
			write(io, string(count, "   3 -1.0 ", vec[1]+0.001, " ", vec[2]+0.001, " ", vec[3]+0.001, "\n"))
			count += 1
			if index_and_charge !== nothing && i in keys(index_and_charge)
				write(io, string(count, "   3 ", Float64(get(index_and_charge, i, -1)), " ", vec[1]+0.001, " ", vec[2]+0.001, " ", vec[3]+0.001, "\n"))
				count += 1
			end
		end
	end
